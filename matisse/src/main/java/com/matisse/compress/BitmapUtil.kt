...
fun getScaledBitmap(
    ...
    var bmp: Bitmap? = BitmapFactory.decodeFile(filePath, options)?.apply {
    ...
    // TODO 2019/12/19 Leo replace Deprecated
    ...
    val scaleMatrix = Matrix().apply { setScale(ratioX, ratioY, 0f, 0f) }
...
private fun generateFilePath(
    ...
    cFileName = if (!TextUtils.isEmpty(cFileName)) cFileName
    else cPrefix + splitFileName(getFileName(context, uri))[0]
